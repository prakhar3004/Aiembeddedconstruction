...
export default function DashboardOverview({ 
  ...
  return (
    <div>
      ...
      {/* Beginner Onboarding Roadmap Section - REDESIGNED */}
      <div className="card" style={{ marginBottom: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', borderBottom: '1px solid var(--border-color)', paddingBottom: '16px' }}>
          <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '18px', margin: 0, fontWeight: 'bold' }}>
            <Compass size={22} style={{ color: 'var(--primary)' }} />
            {language === 'hi' ? 'घर निर्माण यात्रा मार्गदर्शक (AI Groups)' : 'Ghar Nirmaan Journey Guide (AI Groups)'}
          </h3>
          <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
            {language === 'hi' 
              ? '💡 नीचे दिए गए समूहों में से चुनें और चरणों के रहस्य अनलॉक करें।' 
              : '💡 Select a group block below to inspect phases with Civil, Arch & Interior checklists.'}
          </span>
        </div>

        {/* Construction groups cards grid (Non-scrollable, fits mobile & desktop) */}
        <div style={{
          display: 'grid',
          gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))',
          gap: '12px'
        }}>
          {CONSTRUCTION_GROUPS.map((group, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  backgroundColor: isSelected ? 'var(--primary-glow)' : 'rgba(120, 120, 120, 0.015)',
                  display: 'flex',
                  alignItems: 'center',
                  gap: '12px',
              ...
              >
                <div style={{
                  ...
                  height: '36px',
                  borderRadius: '8px',
                  backgroundColor: isSelected ? 'var(--primary)' : 'rgba(120, 120, 120, 0.08)',
                  display: 'flex',
                  alignItems: 'center',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                    {group.phases.length} {language === 'hi' ? 'चरण' : 'steps'}
                  </div>
                </div>
              </div>
            );
        ...
        {/* Selected Group details view: Responsive flex splits */}
        <div style={{
          display: 'flex',
          flexDirection: 'row',
          gap: '24px',
          ...
          <div style={{
            ...
            maxHeight: '340px',
            overflowY: 'auto'
          }} className="phase-side-selector">
            <h4 style={{ fontSize: '12px', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 'bold', margin: '0 0 6px 4px' }}>
              {language === 'hi' ? 'निर्माण चरण' : 'Construction Phases'}
            </h4>
            {selectedGroup.phases.map((phase, pIdx) => {
              const isPhaseSelected = activePhaseIndex === pIdx;
              return (
                <button
                  key={pIdx}
                  ...
                  style={{
                    ...
                    borderRadius: '8px',
                    border: '1px solid',
                    borderColor: isPhaseSelected ? 'var(--primary)' : 'var(--border-color)',
                    backgroundColor: isPhaseSelected ? 'var(--primary-glow)' : 'transparent',
                    color: isPhaseSelected ? 'var(--primary)' : 'var(--text-primary)',
                    fontSize: '12px',
                    fontWeight: isPhaseSelected ? '700' : '600',
          ...
          {/* Phase Details Panels */}
          <div style={{
            flex: '2 1 400px',
            display: 'flex',
            flexDirection: 'column',
            gap: '16px'
          }} className="phase-details-body">
            <div style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '12px' }}>
              <h4 style={{ fontSize: '16px', fontWeight: 'bold', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '6px', margin: 0 }}>
                🚩 {selectedPhase.title[language] || selectedPhase.title.en}
              </h4>
              <p style={{ fontSize: '13px', color: 'var(--text-primary)', marginTop: '8px', lineHeight: '1.5', margin: '8px 0 0' }}>
                <strong>{t.journeyDescLabel}</strong> {selectedPhase.desc[language] || selectedPhase.desc.en}
              </p>
            </div>

            {/* Grid display for the 3 engineering advice zones */}
            <div style={{
              display: 'grid',
              ...
              {/* Civil Card */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', padding: '14px', borderRadius: '8px', border: '1px solid var(--border-color)', backgroundColor: 'rgba(16, 185, 129, 0.03)' }}>
                <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#10b981', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  🏗️ {t.civilTipTitle}
                </span>
                <p style={{ fontSize: '12px', lineHeight: '1.45', margin: 0, color: 'var(--text-primary)' }}>
                  {selectedPhase.civil[language] || selectedPhase.civil.en}
                </p>
              </div>

              {/* Arch Card */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', padding: '14px', borderRadius: '8px', border: '1px solid var(--border-color)', backgroundColor: 'rgba(59, 130, 246, 0.03)' }}>
                <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#3b82f6', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  📐 {t.archTipTitle}
                </span>
                <p style={{ fontSize: '12px', lineHeight: '1.45', margin: 0, color: 'var(--text-primary)' }}>
                  {selectedPhase.arch[language] || selectedPhase.arch.en}
                </p>
              </div>

              {/* Interior Card */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', padding: '14px', borderRadius: '8px', border: '1px solid var(--border-color)', backgroundColor: 'rgba(245, 158, 11, 0.03)' }}>
                <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#f59e0b', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  🎨 {t.interiorTipTitle}
                </span>
                <p style={{ fontSize: '12px', lineHeight: '1.45', margin: 0, color: 'var(--text-primary)' }}>
                  {selectedPhase.interior[language] || selectedPhase.interior.en}
                </p>
              </div>
            </div>

            {/* Embedded AI Advisor prompt trigger */}
            {onAskAI && (
              <button
                ...
                onClick={() => {
                  const queryText = language === 'hi' 
                    ? `मुझे निर्माण समूह "${selectedGroup.title.hi}" के चरण "${selectedPhase.title.hi}" के बारे में पूरी गुणवत्ता जांच गाइड, सामग्री चयन और क्युरिंग अवधि की विस्तृत जानकारी दें।` 
                    : `Provide complete quality standard checks, concrete ratios, curing days and material testing guidelines for Phase: "${selectedPhase.title.en}" under "${selectedGroup.title.en}".`;
                  onAskAI(queryText);
                }}
              >
                <Bot size={16} />
                <span>{language === 'hi' ? 'एआई सलाहकार से विस्तृत प्रश्न पूछें' : 'Consult AI Advisor on this step'}</span>
              </button>
            )}
          </div>
        </div>
